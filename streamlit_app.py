...
def generate_pdf(df, title):
    ...
    for i in range(len(df)):
        row = df.iloc[i]
        text = f"{i+1}. " + " | ".join([f"{col}: {row[col]}" for col in df.columns[:5]])
...
# Sawer Khan ka naam ab list mein shamil hai
    ...
    if menu == "🏠 Ghar ki Entry (Owners)":
        ...
        with st.form("house_form", clear_on_submit=True):
            ...
            if st.form_submit_button("Save Record"):
                payload = {"owner_name": o_name, "contact": o_contact, "location": loc, "portion": portion, "rent": rent, "size": size, "status": status, "added_by": user_name}
    ...
    # --- 7. HOUSE HISTORY (EDIT/DELETE) ---
        ...
        if not df.empty:
            for index, row in df.iterrows():
                with st.expander(f"📍 {row['location']} - {row['owner_name']}"):
